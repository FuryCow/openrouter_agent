...
export function ProjectMemorySettings({
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .catch((err) => {
        setStatus(err instanceof Error ? err.message : t('memory.loadFailed'))
        setStatusSuccess(false)
      })
      .finally(() => setLoading(false))
  }, [workingDirectory, enabled, autoLoadDocs, t])
  ...
  const handleSaveEntries = async (): Promise<void> => {
    ...
    try {
      ...
      setSnapshot(nextSnapshot)
      setStatus(tCommon('toast.memorySaved'))
      setStatusSuccess(true)
    } catch (err) {
      setStatus(err instanceof Error ? err.message : t('memory.saveFailed'))
      setStatusSuccess(false)
    } finally {
      setSaving(false)
    }
    ...
    <div className="space-y-3 rounded-lg border border-white/5 bg-white/[0.02] p-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="text-sm text-zinc-200">{t('memory.title')}</p>
          <p className="text-[11px] text-zinc-500">{t('memory.description')}</p>
        </div>
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="text-sm text-zinc-200">{t('memory.autoLoadDocs.title')}</p>
          <p className="text-[11px] text-zinc-500">{t('memory.autoLoadDocs.description')}</p>
        </div>
        <Switch checked={autoLoadDocs} onCheckedChange={onAutoLoadDocsChange} disabled={!enabled} />
      </div>

      {!workingDirectory ? (
        <p className="text-[11px] text-zinc-500">{t('memory.noWorkspace')}</p>
      ) : (
        <>
          <button
            type="button"
          ...
          >
            ...
            {t('memory.manageEntries')}
            {!loading && existingEntries.length > 0 && (
              <span className="ml-auto text-[10px] text-zinc-500">
                {t('memory.entryCount', { count: existingEntries.length })}
              </span>
            )}
          </button>
            ...
            <div className="space-y-3 rounded-lg border border-white/5 bg-black/10 p-3">
              <div>
                <label className="mb-1.5 block text-xs font-medium text-zinc-400">
                  {t('memory.snapshotPreview')}
                </label>
                <textarea
                  readOnly
                  value={loading ? tCommon('status.loading') : snapshot}
                  rows={3}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={t('memory.entryPlaceholder')}
                  className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs text-zinc-200"
                />
                <p className="mt-1 text-[10px] text-zinc-500">{t('memory.entryHint')}</p>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {saving ? tCommon('actions.saving') : t('memory.saveMemory')}
                </Button>
                {status && (
                  <span
                    className={cn(
                      'text-[11px]',
                      statusSuccess ? 'text-emerald-400' : 'text-zinc-500'
                    )}
                  >
                    {status}
                  </span>
